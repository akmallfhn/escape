...
const cards: CardData[] = [
  {
    ...
    features: ["Challenge Card", "Wristband Regular", "Regular Seat"],
    price: "Rp. 250.000",
  ...
  },
  {
    ...
    features: ["Challenge Card", "Wristband VIP", "VIP Seat"],
    price: "Rp. 350.000",
...
function CheckCircle({ dark }: { dark: boolean }) {
  return (
    <span
      className={`mt-0.5 size-4.5 rounded-full flex items-center justify-center shrink-0 ${
...
function TicketCard({ card, index }: { card: CardData; index: number }) {
  const text = card.dark ? "text-white" : "text-black";
  const ref = useRef<HTMLDivElement>(null);
  const [phase, setPhase] = useState<"hidden" | "image" | "text">("hidden");
      ...
      ([entry]) => {
        if (entry.isIntersecting) {
          ...
          cardTimer = setTimeout(() => {
            setPhase("image");
            textTimer = setTimeout(() => setPhase("text"), 600);
          }, cardDelay);
        } else {
  ...
  return (
    ...
    >
      <div className="flex ml-2 flex-col items-stretch px-7 pt-[16%] pb-6">
        <div
          className={`flex flex-col gap-2.5 ${text}`}
          style={{
        ...
        >
          ...
          <div
            className={`h-px w-4/5 my-1 ${
              card.dark ? "bg-white/30" : "bg-black/25"
            }`}
          />
            ...
            {card.features.map((f, i) => (
              <li
                ...
                style={{
                  opacity: phase === "text" ? 1 : 0,
                  transform:
                    phase === "text" ? "translateY(0)" : "translateY(10px)",
                  transition:
                    phase === "text"
                      ? `opacity 0.4s ease ${i * 60 + 80}ms, transform 0.4s ease ${i * 60 + 80}ms`
                      : "none",
                }}
              >
          ...
          <div
            ...
            style={{
              ...
              transition:
                phase === "text"
                  ? `opacity 0.4s ease ${card.features.length * 60 + 140}ms, transform 0.4s ease ${card.features.length * 60 + 140}ms`
                  : "none",
            }}
          >
  ...
  return (
    <section className="bg-black w-full flex items-center justify-center py-16 px-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full max-w-5xl lg:max-w-6xl">
        {cards.map((card, i) => (
          <TicketCard key={card.tier} card={card} index={i} />
        ))}
      </div>
    </section>
  );
